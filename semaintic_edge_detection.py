...
import numpy as np
import os
from DataGenEdge import DataGen
...
fg_images = np.array(dataG.load_instance_labels())
        ...
        for batch_count in range(BATCH_COUNT):
            ...
            if epoch == (config_etc.TOTAL_EPOCH - 1):
                for index, image in enumerate(image_result_predict):
                    ...
                    scipy.misc.imsave(
                        '/data1/LJH/cvpppnet/A1_predict_edge/plant_out_epc{}_{}.png'.format(epoch + 1, suffix),
                        np.squeeze(image))
